...
A companion package, `src/Dynamics365.BusinessCentral.Testing/` (`FakeBusinessCentral`), runs a real client over a scripted transport so consumers can assert the exact OData their code produces. It is versioned in lockstep with the main csproj and ships from the same release. Two rules: it depends only on the main package's **public** API (no `InternalsVisibleTo` — if the fake needs something internal, that something probably wants to be public; this is why `EntitySelect` is public), and it deliberately contains **no OData query evaluation** — an in-memory `$filter` engine would be a permanent compatibility liability. Keep it a transport fake. Its contract is pinned by `FakeBusinessCentralTests` in the main test project.

The package also ships `BusinessCentralMetadata` (M4), the one check a transport fake structurally cannot do: it fetches a tenant's `$metadata` via `IBusinessCentralClient.GetMetadataAsync` and verifies every `[BusinessCentralEntity]` type's derived `$select` resolves to real columns. `Parse`/`Validate` are pure and unit-tested against a canned EDMX fixture; only the fetch needs a tenant — that split is what let it ship in 2.0.0 beside the F2 behaviour it protects. It reports **all** problems, never the first. Column matching is `OrdinalIgnoreCase` **on purpose** — BC's `$select` is case-insensitive, so an ordinal match would flag working projections as broken.
...
# Live-tenant facts (net10.0 only, read-only). Needs BOTH the opt-in and credentials —
# without either, every live fact skips. The opt-in is what keeps a solution-wide
# `dotnet test` from becoming a live-tenant run on a machine that has .env/dev-tenant.md.
...
Every URL the builder assembles passes through `Finish` (schema version, then `Guard`) or, for `BuildQueryUrl`, composes `$schemaversion` into its own query list before guarding. **All six builders must emit the schema version** — it first shipped on `BuildQueryUrl` alone, which left reads-by-key, writes, the company list, raw queries and `$metadata` running under a different contract from list queries, silently and per method.

`Guard` measures the **query string, not the full URL**. Measured against a live SaaS tenant: the gateway accepts 8,099 query-string characters and that ceiling is invariant across environments, while the full URL moves with environment name, company name (`Company('KRAL%20AG')`) and entity-set path — so no full-URL default is portable. Defaults are `MaxQueryStringLength` 8000 / `QueryStringLengthWarningThreshold` 6000, both from that measurement. Over-length gets `414 URI Too Long` from the server, not an opaque `400`; the guard's value is pre-flight diagnosis (which filter, how many `or` clauses, `Filter.In` as the cause), not decoding an unhelpful status. The builder is also the right seam because a server-issued `@odata.nextLink` never passes through it — `FetchNextPageAsync` sends the absolute URL verbatim — so continuations are exempt for free. Public entry points guard exactly once each; the internal `EntityUrl` helpers exist so composed builders don't double-report.
...
`Filter.In` is the one filter whose text is **not** decided at construction. `ODataInStyle.Auto` (the default) defers to the client, which renders native `in` when `BusinessCentralOptions.UseNativeIn` resolves true (schema version ≥ 2.1, or `InStyle` forcing it) — BC gates `in` on 2.1 and answers `501` below it. `ODataFilter` therefore carries an optional `Func<bool, string>` alongside its eager `Value`, and `FilterExtensions.Combine`/`Not` **must** propagate it: a chunked key lookup is nearly always composed, so dropping the deferral would make the feature inert exactly where it pays. `Value` stays the portable or-chain because a bare value has no endpoint to ask — the only place wire form and `Value` legitimately differ. Both surfaces resolve it: `BusinessCentralQuery.FilterValue` via `_executor.UseNativeIn`, path-based via `BusinessCentralClient.RenderFilter`.
...
`test/Dynamics365.BusinessCentral.LiveTenant.Tests/` — the other half, and the only tests that touch a real tenant. Everything above runs through `FakeHttpHandler`, which proves what the client *sends*; these prove Business Central accepts it, and notice when BC changes underneath the package. **`net10.0` only** — one round trip per fact is enough, and what differs between TFMs is the compiled surface, which the gate and package validation already cover. That single TFM is why `sonar.yml`'s coverage step is scoped to the unit project: `-f net8.0` over the whole solution no longer restores. The live project is nevertheless **built inside the scanner interval** — SonarCloud analyzes only what is compiled between `begin` and `end`, and nothing else references it, so without that line its sources would be absent from analysis and from the quality gate while everything still looked green. **No `InternalsVisibleTo`** — this project is a consumer, so anything it proves is something a consumer can rely on. Facts skip (never fail) without credentials, and `live-tenant.yml` therefore asserts **all three** credentials exist before running — an unset repository variable skips the suite as effectively as a missing secret. It runs on push-to-master and a weekly schedule only: not on `pull_request` (secrets are unavailable to fork PRs; `pull_request_target` is the classic leak) and **not on `workflow_dispatch`**, because a dispatch runs the workflow definition from the ref it is given, so a pushed branch could edit the job and read the secret out of an unreviewed run — an in-file branch guard cannot fix that, since the attacker's copy is the one that runs. Restoring manual runs needs a GitHub Environment with a master-only deployment-branch policy. The job fixes `TZ=Europe/Vienna`: the kindless-DateTime regression is specifically a machine-local conversion, and an ordinary UTC runner makes old and new behavior identical. That guarantee is one deleted YAML line thick, so `DateFilterTests.RequireADiscriminatingClock` **fails** both date facts when a `GITHUB_ACTIONS` run has a UTC clock — a contributor under UTC still passes with a printed note, because they have nothing to fix and the workflow does. `LiveTenant.GuardEnvironment` checks **authority before path**: HTTPS, default port, and host equal to `LiveTenant.Host`, because every request these tests make carries a real token and the app registration's sandbox grant constrains which environment a token opens, not who receives it — `https://attacker.example/v2.0/{tenant}/KRALTEST/ODataV4` satisfies any path-only check. It is the same rule `BusinessCentralClient.EnsureTrustedContinuation` applies to `@odata.nextLink`, and host **equality**, since `api.businesscentral.dynamics.com.evil.test` passes a suffix test. It then reads the environment out of the **position** BC puts it in — the segment immediately before `ODataV4` — and rejects any shape it cannot parse. Two weaker forms were both wrong the same way: a substring test accepts `KRALTEST2`, and "the name in any segment" accepts `https://proxy/KRALTEST/v2.0/{tenant}/Production/ODataV4`. `LiveTenantCredentials` stops its parent walk at the repo root so a sibling project's `.env/` cannot be picked up. Live facts need **both** credentials and the `BC_LIVE_TENANT_TESTS` opt-in (`LiveTenant.OptInVariable`): the project stays in the `.slnx` so it keeps being maintained, and the opt-in is what stops a solution-wide `dotnet test` becoming a live run. `GuardEnvironmentTests` needs no tenant and always runs. Every fact that compares two live reads **brackets** them through `LiveTenantAssert.WithinBracket` — the moving quantity is measured either side of the thing under test and the result must land inside — because an exact comparison against a single count flakes on concurrent tenant activity in exactly the shape of a real regression; that covers the paging facts (`$count` around the paged read) and both date facts. The machine-local one checks `local == kindless ± gap` — plus when the local interpretation moves the boundary earlier, minus when it moves it later — and therefore brackets **both** populations around the reading under test: the rows above the boundary *and* the rows inside the timezone window. Under the workflow's positive offset, a row inserted inside the window changes the gap while every `>= boundary` count stays put, so a tolerance drawn from those counts alone would measure zero exactly when it was needed and fail the fact with the conversion working. Everything both date facts can detect is the rows that window holds, so `FindBoundaryAsync` **places** the boundary — offsetting the sampled row's timestamp by the direction that applies — rather than sampling one and hoping: an empty window makes the 1.0 and 2.0 readings identical and the whole class passes with the conversion restored. Placement took the measured window from 1 row to 16. A sample the drift outweighs is **inconclusive and fails**, because a fact that cannot fail must not report the same green as one that can. `SetWithinBracket` is the same instrument for identity rather than size — used by the native-`in` fact, which reads the or-chain either side of the native one — and its two halves are deliberately asymmetric: a row **both** reference reads saw is *required*, a row only one saw is *permitted*, and a row neither saw is the finding. Both brackets collapse to an exact comparison on a quiet tenant, and both print what they measured so a widening bracket is visible rather than silently absorbed. `LiveTenantAssertTests` pins those semantics and needs no tenant. Each paging fact also proves paging **happened**: a set that arrived in one response satisfies every row-level assertion while exercising no continuation. The server-driven fact holds a first page and then records `ToAllAsync` itself, requiring its follow-up request to carry BC's opaque `$skiptoken` and no client-generated `$skip`; the page-preference fact counts requests through a `RecordingObserver`, because `ToPageAsync` deliberately sends no `odata.maxpagesize` (it would truncate a one-shot read) and *n* rows cannot arrive in fewer than *n*/pageSize requests. `ProjectionTests` reads the canonical column spellings out of `$metadata`, compares **ordinally**, and requires the wrong-case `$select` response to contain exactly the requested properties (beside OData annotations) — the combined claim is that BC both applies the projection and answers it in metadata casing. `QueryStringCeilingTests` sends adjacent fixed-width filter shapes at 8,062 and 8,126 characters, the narrowest interval that shape can express around the measured 8,099 boundary. The suite is **read-only**: the write path is covered against the same tenant by the consuming application's live suite. Numbers the tenant owns (server page size, row counts) are reported, not asserted — pinning them would turn an administrative change into a package regression.
