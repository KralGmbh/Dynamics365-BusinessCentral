...
`EntitySelect` cannot validate against the tenant, so a derived name that isn't a real column fails the query with a `400`. Two distinct cases, and don't conflate them in docs: casing drift was *already* broken (the property never bound), but a property with no matching column at all is breakage the derivation **creates** — it used to bind as its default. `BusinessCentralQuery.ExecutePageAsync` therefore routes every fetch through `DerivedSelectHint`, which re-wraps a `400` with the derivation, the implicated property and both remedies. Re-wrapping needs `BusinessCentralException.ServerMessage` (the undecorated message) or the `(GET → HTTP 400)` suffix accumulates. `UsesDerivedSelect` gates it, so explicit `Select`, `SelectAll`, `CountAsync` and the path-based API never see the hint.
